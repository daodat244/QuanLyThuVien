...
import Control.NhanVienDAO; // Giả sử có lớp này
import Model.Sach;
import Model.DocGia;
...
private void searchPhieuMuon() {
        try {
            ...
            if (keyword.isEmpty()) {
                phieuMuonList = phieuMuonDAO.getAllPhieuMuonWithDetails(); // Nếu từ khóa rỗng, hiển thị toàn bộ
            } else {
                phieuMuonList = phieuMuonDAO.searchPhieuMuon(keyword); // Tìm kiếm theo từ khóa
            }
        ...
        try {
            ...
            for (PhieuMuon pm : phieuMuonList) {
                ...
                model.addRow(new Object[]{
                    ...
                    pm.getNgaymuon(),
                    pm.getNgaytra(),
                    pm.getTrangthai() // Thêm cột trạng thái vào bảng
                });
            }
        } catch (SQLException ex) {
...
try {
            ...
            LocalDateTime ngayHenTra = date1.getDateTimePermissive();
            LocalDateTime ngayMuon = date.getDateTimePermissive();

            if (maSachText.isEmpty() || maDocGiaText.isEmpty() || maNhanVienText.isEmpty() || ngayMuon == null || ngayHenTra == null) {
            ...
            if (ngayMuon.isAfter(ngayHenTra)) {
            ...
            pm.setNgaytra(ngayHenTra);
            pm.setTrangthai("Chưa trả"); // Tự động đặt trạng thái là "Chưa trả"
...
int row = jTable1.getSelectedRow();
        ...
        try {
            ...
            pm.setNgaymuon(ngayMuon);
            pm.setNgaytra(ngayHenTra);
            // Giữ trạng thái hiện tại từ bảng hoặc đặt "Chưa trả" nếu không có
            String currentTrangthai = (String) jTable1.getValueAt(row, 6);
            pm.setTrangthai(currentTrangthai != null ? currentTrangthai : "Chưa trả");
